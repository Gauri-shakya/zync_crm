...
class SalaryController extends Controller
{
    ...
    {
        $salary = $this->baseSalaryQuery()->with(['employee.company'])->findOrFail($id);
        
        // Auto-recalculate ONLY if the record is "Fresh" (never manually updated)
        // OR if the summary is mathematically broken (doesn't add up to company working days)
        $company = $salary->employee->company;
        $workingDays = $company->total_working_days ?? 22;
        $currentSum = $salary->total_present_days + $salary->total_absent_days + $salary->total_half_days + $salary->total_late_days;

        // If it hasn't been manually edited yet (updated_at == created_at)
        // OR if the working days don't match (e.g. record #6 with old logic)
        if ($salary->updated_at->eq($salary->created_at) || $currentSum != $workingDays) {
            $date = Carbon::parse($salary->salary_month);
            $calculated = $salary->calculateSalaryData($salary->employee, $date->month, $date->year, $salary->basic_salary);
            
            // Show the "Corrected" version on first load
            $salary->total_absent_days = $calculated['total_absent_days'];
            $salary->per_day_salary = $calculated['per_day_salary'];
            $salary->net_salary = $calculated['net_salary'];
        }
    ...
    public function recalculate($id)
    {
        ...
        $calculatedData['attendance_summary'] = [
            'total_working_days' => $calculatedData['total_working_days'] ?? (Company::find($salary->employee->company_id)->total_working_days ?? 22),
