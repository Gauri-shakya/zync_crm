...
class TicketController extends Controller
{
    ...
    public function getMessages($id)
    {
        ...
        $conversations = array_filter($ticket->conversations, function($msg) {
            return !isset($msg['is_internal']) || !$msg['is_internal'];
        });

        // Add formatted time and attachment URLs
        $conversations = array_map(function($msg) {
            $msg['formatted_time'] = \Carbon\Carbon::parse($msg['created_at'])->format('h:i A');
            if (isset($msg['attachments'])) {
                foreach ($msg['attachments'] as &$att) {
                    $att['url'] = Storage::url($att['path']);
                }
            }
            return $msg;
        }, $conversations);
