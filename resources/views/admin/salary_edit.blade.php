                ...
                <div class="bg-white overflow-hidden shadow-sm rounded-lg sm:rounded-lg">
                    ...
                    <form action="{{ route('salary.update', $salary->id) }}" method="POST" class="p-4 md:p-6 space-y-4 md:space-y-6">
                        ...
                        <div>
                            ...
                            <input type="number" name="net_salary" value="{{ $salary->net_salary }}" step="0.01" disabled class="disabled:bg-gray-100 disabled:text-gray-500 w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 font-bold text-lg">
                        </div>
                 ...
                 <div class="bg-white overflow-hidden shadow-sm rounded-lg sm:rounded-lg sticky top-6">
                    ...
                    <div class="p-4 md:p-6 space-y-4">
                         <div class="flex justify-between items-center pb-2 border-b border-gray-50">
                            <span class="text-gray-600 font-medium">Total Working Days</span>
                            <span id="summary_working_days" class="font-bold text-indigo-600">{{ $salary->employee->company->total_working_days ?? '--' }}</span>
                        </div>
...
<script>
    ...
    const salaryInputs = ['basic_salary', 'total_allowances', 'total_deductions', 'overtime_amount'];
    const netSalaryInput = document.querySelector('input[name="net_salary"]');
    let isManualNetSalary = false;

    // Detect manual override of Net Salary
    netSalaryInput.addEventListener('input', () => {
        isManualNetSalary = true;
    });

    function calculateNetSalary() {
        if (isManualNetSalary) return;

        const basicRate = parseFloat(document.querySelector('input[name="basic_salary"]').value) || 0;
        const allowances = parseFloat(document.querySelector('input[name="total_allowances"]').value) || 0;
        const deductions = parseFloat(document.querySelector('input[name="total_deductions"]').value) || 0;
        const overtime = parseFloat(document.querySelector('input[name="overtime_amount"]').value) || 0;

        // Get Attendance Summary Data
        const totalWorkingDays = parseFloat(document.getElementById('summary_working_days')?.innerText) || 25;
        const presentDays = parseFloat(document.getElementById('summary_present')?.innerText) || 0;
        const halfDays = parseFloat(document.getElementById('summary_half')?.innerText) || 0;
        const lateDays = parseFloat(document.getElementById('summary_late')?.innerText) || 0;
        ...
        const perDaySalary = basicRate / totalWorkingDays;
        
        // Calculate earned base salary: (Present + Late) * PerDay + (Half * 0.5 * PerDay)
        // Note: Assuming 50% for half days as per standard
        const earnedBasic = (presentDays + lateDays) * perDaySalary + (halfDays * 0.5 * perDaySalary);

        const net = (earnedBasic + allowances + overtime) - deductions;
                ...
                fetch(`{{ route('salary.recalculate', $salary->id) }}`)
                    ...
                    .then(result => {
                        if (result.success) {
                            ...
                            // Reset manual override flag since we explicitly requested a recalculation
                            isManualNetSalary = false;
                            
                            // Update attendance summary if elements exist
                            if(document.getElementById('summary_working_days')) document.getElementById('summary_working_days').innerText = data.total_working_days;
