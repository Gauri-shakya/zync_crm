...
@section('hrms-content')
    <div class="container mx-auto px-3 sm:px-4 py-4 sm:py-8">
        ...
        <div class="bg-white shadow-xl rounded-lg overflow-hidden border border-gray-200">
            ...
            <div
                class="bg-gradient-to-r from-purple-600 to-indigo-600 text-white px-4 sm:px-6 py-4 sm:py-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-0">
                ...
                <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                    <button onclick="openImportModal()"
                        class="bg-white text-purple-600 px-4 py-2 rounded-lg text-xs sm:text-sm font-semibold hover:bg-gray-100 transition-colors shadow-sm w-full sm:w-auto text-center">
                        <i class="fas fa-file-import mr-2"></i>Import Excel
                    </button>
                    <a href="{{ route('attendance-record.index') }}"
                        class="text-xs sm:text-sm font-medium hover:text-white/90 transition-colors underline w-full sm:w-auto text-center sm:text-right flex items-center justify-center sm:justify-end">
                        <i class="fas fa-sync-alt mr-2 sm:hidden"></i>Refresh Data
                    </a>
                </div>
            </div>
            ...
            <div class="p-4 sm:p-6">
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex-grow">
                                ...
                                <p class="text-2xl sm:text-3xl font-bold text-gray-900 mt-1">
                                    {{ $totalSummary['total_employees'] ?? 0 }}</p>
                                <p class="text-xs text-green-600 mt-2 hidden sm:block">↑ 12.5% vs last month</p>
                            </div>
                            <div
                                class="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-green-100 flex items-center justify-center flex-shrink-0 ml-3">
                    ...
                    <div
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex-grow">
                                ...
                                <p class="text-2xl sm:text-3xl font-bold text-gray-900 mt-1">
                                    {{ $totalSummary['present_today'] ?? 0 }}</p>
                                <p class="text-xs text-green-600 mt-2 hidden sm:block">↑ 8.2% vs yesterday</p>
                            </div>
                            <div
                                class="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-blue-100 flex items-center justify-center flex-shrink-0 ml-3">
                    ...
                    <div
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex-grow">
                                ...
                                <p class="text-2xl sm:text-3xl font-bold text-gray-900 mt-1">
                                    {{ $totalSummary['total_clients'] ?? 0 }}</p>
                                <p class="text-xs text-green-600 mt-2 hidden sm:block">↑ 15.3% vs last month</p>
                            </div>
                            <div
                                class="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-purple-100 flex items-center justify-center flex-shrink-0 ml-3">
                    ...
                    <div
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex-grow">
                                ...
                                <p class="text-2xl sm:text-3xl font-bold text-gray-900 mt-1">
                                    {{ $totalSummary['total_contacts'] ?? 0 }}</p>
                                <p class="text-xs text-red-600 mt-2 hidden sm:block">↓ 3.1% vs last month</p>
                            </div>
                            <div
                                class="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-orange-100 flex items-center justify-center flex-shrink-0 ml-3">
                ...
                <div class="mb-6 sm:mb-8 p-4 sm:p-5 bg-gray-50 rounded-xl border border-gray-200">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        {{-- Month Filter --}}
                        <div>
                            <label for="monthFilter" class="block text-xs sm:text-sm font-semibold text-gray-700 mb-1 sm:mb-2">Select Month</label>
                            <form action="{{ route('attendance-record.index') }}" method="GET" class="w-full flex gap-2">
                                <input type="month" name="month" id="monthFilter" value="{{ $currentMonthYear === 'All Time' ? '' : $currentMonthYear }}" 
                                    class="w-full border border-gray-300 rounded-lg px-3 sm:px-4 py-2.5 sm:py-3 text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 bg-white"
                                    onchange="this.form.submit()">
                                @if($currentMonthYear !== 'All Time')
                                    <button type="button" onclick="window.location.href='{{ route('attendance-record.index', ['month' => 'all']) }}'" 
                                        class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-3 py-2 rounded-lg text-xs sm:text-sm whitespace-nowrap transition-colors">
                                        Show All
                                    </button>
                                @else
                                    <button type="button" onclick="window.location.href='{{ route('attendance-record.index') }}'" 
                                        class="bg-purple-600 hover:bg-purple-700 text-white px-3 py-2 rounded-lg text-xs sm:text-sm whitespace-nowrap transition-colors">
                                        Show Current
                                    </button>
                                @endif
                            </form>
                        ...
                        <div>
                            <label for="employeeFilter"
                                class="block text-xs sm:text-sm font-semibold text-gray-700 mb-1 sm:mb-2">Filter by
                                Employee</label>
                            <div class="flex flex-col sm:flex-row gap-3 sm:gap-4">
                                <select id="employeeFilter"
                                    class="flex-grow border border-gray-300 rounded-lg px-3 sm:px-4 py-2.5 sm:py-3 text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 bg-white">
                                ...
                                <button type="button" id="clearFilters"
                                    class="bg-gray-700 hover:bg-gray-900 text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg text-xs sm:text-sm font-medium transition-colors shadow-sm w-full sm:w-auto">
                                    Clear
                                </button>
                            </div>
                        </div>
                        ...
                        <div class="w-full">
                            <table id="attendanceTable" class="w-full table-auto text-xs sm:text-sm">
                                <thead class="bg-gray-50">
                                    <tr>
                                        <th
                                            class="px-3 sm:px-6 py-3 text-left font-bold text-gray-700 uppercase tracking-wider">
                                            Employee</th>
                                        <th
                                            class="px-3 sm:px-6 py-3 text-left font-bold text-gray-700 uppercase tracking-wider">
                                            Total Present</th>
                                ...
                                <tbody class="divide-y divide-gray-100">
                                    @foreach($employeeSummaries as $employeeSummary)
                                        <tr class="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td class="px-3 sm:px-6 py-4 text-gray-900 text-center">
                                                {{ $employeeSummary['present_count'] }}</td>
                                            <td class="px-3 sm:px-6 py-4 text-gray-900 text-center">
                                                {{ $employeeSummary['absent_count'] }}</td>
                                            <td class="px-3 sm:px-6 py-4 text-gray-900 text-center">
                                                {{ $employeeSummary['half_day_count'] }}</td>
                                            <td class="px-3 sm:px-6 py-4 text-center">
                                                <button
                                                    class="bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white px-3 sm:px-5 py-2 sm:py-2.5 rounded-lg text-xs sm:text-sm font-medium transition-all shadow-sm hover:shadow viewMonthly w-full sm:w-auto"
                                                    data-employee-id="{{ $employeeSummary['employee_id'] }}"
                                                    data-employee-name="{{ $employeeSummary['employee_name'] ?? 'N/A' }}"
                                                    data-attendance-date="{{ $currentMonthYear === 'All Time' ? 'all' : $currentMonthYear . '-01' }}">
                                                    View Details
                                                </button>
                                            </td>
                                        </tr>
    ...
    <div id="monthlyModal"
        class="fixed inset-0 bg-black bg-opacity-50 overflow-y-auto h-full w-full hidden z-50 flex items-center justify-center p-2 sm:p-4">
        <div class="relative mx-auto p-4 sm:p-6 md:p-8 w-full max-w-6xl max-h-[90vh] sm:max-h-[85vh] overflow-y-auto bg-white rounded-xl sm:rounded-2xl shadow-2xl"
            id="modalContent">
            ...
            <div
                class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4 mb-4 sm:mb-6 p-3 sm:p-5 bg-gray-50 rounded-xl border border-gray-200">
                <div class="sm:col-span-2 lg:col-span-1">
                    <label for="modalEmployeeFilter" class="block text-xs font-semibold text-gray-700 mb-1">Employee</label>
                    <input type="text" id="modalEmployeeFilter"
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-xs sm:text-sm bg-gray-100" disabled>
                </div>
                <div>
                    <label for="modalStatusFilter" class="block text-xs font-semibold text-gray-700 mb-1">Status</label>
                    <select id="modalStatusFilter"
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-purple-500">
                ...
                <div class="flex items-end">
                    <button type="button" id="modalClearFilters"
                        class="w-full bg-gray-700 hover:bg-gray-900 text-white px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium transition-colors shadow-sm">
                        Clear Filters
                    </button>
                </div>
            </div>
    ...
    <style>
        ...
        @media (max-width: 768px) {
            ...
            #monthlyDataTable button,
            #monthlyDataTable .status-badge {
                font-size: 0.65rem;
                line-height: 1.2;
            }
        }
    ...
    <div id="importModal"
        class="fixed inset-0 bg-black bg-opacity-50 overflow-y-auto h-full w-full hidden z-50 flex items-center justify-center p-4">
        <div class="relative mx-auto p-6 w-full max-w-md bg-white rounded-xl shadow-2xl">
            <div class="flex justify-between items-center pb-4 border-b border-gray-200 mb-4">
                <h5 class="text-xl font-bold text-gray-900">Import Attendance</h5>
                <button type="button" class="text-gray-400 hover:text-gray-600" onclick="closeImportModal()">
                    <i class="fas fa-times text-xl"></i>
                </button>
            </div>

            <form action="{{ route('attendance-record.import') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="mb-4">
                    ...
                    <div id="fileName" class="mt-2 text-sm text-gray-600 text-center hidden"></div>
                </div>

                <div class="flex justify-between items-center mt-6">
                    <a href="{{ route('attendance-record.sample') }}" class="text-sm text-purple-600 hover:text-purple-800 font-medium">
                        <i class="fas fa-download mr-1"></i> Download Sample
                    </a>
                    <button type="submit"
                        class="bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white px-6 py-2 rounded-lg font-medium shadow-sm transition-all">
                        Import
                    </button>
                </div>
            </form>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $(document).on('click', '.viewMonthly', function () {
                ...
                let attendanceDate = $(this).data('attendance-date');
                let selectedMonth = attendanceDate === 'all' ? 'all' : attendanceDate.substring(0, 7);

                $('#monthlyEmployeeName').text(employeeName + ' (' + (selectedMonth === 'all' ? 'All Time' : selectedMonth) + ')');
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        monthlyData.forEach(function (item) {
                            ...
                            // Get status class
                            let statusRaw = (item.status || '').trim();
                            // Normalize spaces
                            statusRaw = statusRaw.replace(/\s+/g, ' ');
                            let statusLower = statusRaw.toLowerCase();
                            let statusClass = 'status-badge ';
                            
                            // Normalize status for display (Capitalize first letter)
                            let displayStatus = statusRaw.charAt(0).toUpperCase() + statusRaw.slice(1).toLowerCase();
                            if (statusLower === 'half day') displayStatus = 'Half Day';
            ...
            // File input change handler for Import Modal
            $('#dropzone-file').change(function() {
                if (this.files && this.files[0]) {
                    $('#fileName').removeClass('hidden').text('Selected: ' + this.files[0].name);
                }
            });
