        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2">
                <div class="bg-white rounded-xl shadow-sm overflow-hidden mb-6">
                    <!-- Header -->
                    <div class="p-4 sm:p-6 border-b border-gray-100 bg-gray-50 flex flex-col sm:flex-row justify-between items-start gap-4">
                        <div class="w-full sm:w-auto">
                            <div class="flex items-center gap-3 mb-2">
                                <span class="text-xs font-bold text-gray-500 uppercase tracking-wider">Ticket
                                    #{{ $ticket->ticket_id }}</span>
                            </div>
                            <h1 class="text-xl font-bold text-gray-900 break-words">{{ $ticket->title }}</h1>
                        </div>
                        <div class="w-full sm:w-auto sm:text-right border-t sm:border-t-0 pt-3 sm:pt-0">
                            ...
                            <div class="text-xs text-gray-500 truncate">{{ $ticket->client->email ?? '' }}</div>
                        </div>
                    </div>
...
@endphp
                    ...
                    <div class="p-4 sm:p-6 bg-white border-t border-gray-100">
                        <form action="{{ route('ticket.record.reply', $ticket->id) }}" method="POST"
                            ...
                            <div
                                class="flex items-center mb-4 bg-yellow-50 p-3 rounded-lg border border-yellow-100 w-full sm:w-fit max-w-full">
                                <input type="checkbox" id="is_internal" name="is_internal" value="1"
                                    class="h-5 w-5 rounded border-gray-300 text-yellow-600 shadow-sm focus:border-yellow-300 focus:ring focus:ring-yellow-200 focus:ring-opacity-50">
                                <label for="is_internal"
                                    class="ml-3 block text-sm text-gray-700 cursor-pointer font-medium select-none truncate">
                            ...
                            <div class="mb-4">
                                <textarea name="message" rows="4" placeholder="Type your reply here..."
                                    class="w-full rounded-lg border-gray-300 border p-3 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-shadow outline-none text-base sm:text-sm"
                                    required></textarea>
                            </div>

                            <div class="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4">
                                <label
                                    class="cursor-pointer text-gray-500 hover:text-blue-600 transition-colors flex items-center gap-2 text-sm bg-gray-50 hover:bg-gray-100 p-2.5 rounded-lg border border-dashed border-gray-300 justify-center sm:justify-start">
                                    <i class="fas fa-paperclip text-lg"></i>
                                    <span class="font-medium">Attach Files</span>
                                    <input type="file" name="attachments[]" multiple class="hidden">
                                </label>
                                <button type="submit"
                                    class="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 rounded-lg font-bold transition-all shadow-md active:scale-95 text-base sm:text-sm">
