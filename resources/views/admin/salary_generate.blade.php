...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        bulkGenerateBtn.addEventListener('click', async function() {
            Swal.fire({
                title: 'Bulk Generate Salary',
                text: 'Generate salaries for ALL employees for selected month? This may take a moment.',
                icon: 'question',
                ...
                if (result.isConfirmed) {
                    ...
                    try {
                        ...
                        if (data.success) {
                            ...
                            let resultMsg = `Successfully generated ${successCount} salaries.`;
                            if (skippedCount > 0) resultMsg += ` ${skippedCount} were already generated.`;
                        ...
                        Swal.fire({
                            title: 'Network Error',
                            text: 'The request took too long or there was a connection issue. Please check the Salary Records list to see if they were generated.',
                            icon: 'warning'
                        });
                    }
                }
        ...
        function showBulkSuccess(data) {
            const resultDiv = document.getElementById('result');
            if (!resultDiv) return;

            let html = `
                <div class="bg-white overflow-hidden shadow-sm rounded-lg border border-blue-200">
                    <div class="p-4 md:p-6 bg-blue-50">
                        <h3 class="text-lg font-bold text-gray-800">Bulk Generation Results</h3>
                    </div>
            ...
            data.results.forEach(res => {
                const statusClass = res.status === 'success' ? 'bg-green-100 text-green-800' : 
                                  (res.status === 'skipped' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800');
                
                html += `
                    <tr>
                        <td class="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900">${res.employee}</td>
                        <td class="px-4 py-3 whitespace-nowrap">
                            <span class="px-2 py-0.5 rounded-full text-xs font-bold ${statusClass}">${res.status.toUpperCase()}</span>
                        </td>
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500">
                            ${res.status === 'success' ? '₹' + res.net_salary.toLocaleString() : (res.message || res.error || '-')}
                        </td>
                    </tr>
                `;
            });
